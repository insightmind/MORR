...
\begin{parameters}
\para{callback : function(Shared.BrowserEvent) => void}{The function to invoke on created events.}
\end{parameters}
\end{constructor}
\end{constructors}
\begin{methods}
...
\para{windowID : number}{The ID of the window the tab with tabID belongs to.}
\para{newTabID}{The ID of the newly focused tab.}
...
\para{target}{String describing the target element which received the input, i. e. a textbox.}
\para{url : string}{The URL of the open webpage.}
\end{parameters}
\end{constructor}
\end{constructors}
...
\clsdiagram{resources/Classes/BrowserExtension/Listeners/DownloadListener.png}
