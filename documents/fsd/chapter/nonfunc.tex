...
\nfspec{CPU Performance}{
    The CPU utilization by the application on a \gls{user}'s \gls{device} with an Intel Core i5 8250U or better and a screen resolution of 1080p or smaller should not exceed 25\% while recording a \gls{session}.
}
...
\nfspec{Start Response Performance}{
	The delay between the \gls{user} starting a \gls{session} via hitting the button and the actual \gls{session} start should not exceed 1 second.
}
...
\nfspec{Offline Recordings}{
	The \gls{user} can record a \gls{session} and save sessions locally without being connected to the internet.
}
