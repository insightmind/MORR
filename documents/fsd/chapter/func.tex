...
\begin{itemize}
    ...
    \fspec{Event types}{\Gls{user}}{\sessionactive}{When the \gls{user} triggers an \gls{event} specified in chapter \ref{ch:data}, the respective \gls{module} (see \ref{sec:modules}) generates a data-structure (as defined in \ref{sec:events}) associated with the \gls{event} and adds the event-type to it.}{The data structure associated with the \gls{event} is sent to the data-managing component and contains a field describing its \gls{event}-type.}
    \fspec{Event processing}{\Gls{user}}{\sessionactive}{When the \gls{user} triggers an \gls{event} specified in chapter \ref{ch:data}, the data structure corresponding to this \gls{event} is processed and filtered by \glspl{module} (see \ref{sec:modules} and \ref{sec:sysarchitecture}). If the configuration does not specify to discard the \gls{event}, the \gls{event} data structure is sent to the data-managing component by the respective \gls{module}.}{The \gls{event} data structure is either sent to the data-managing component for serialization or deleted based on a configured rule.}
    ...
    \fspec{\Gls{user} instruction}{\Gls{user}}{\applaunched{} \sessioninactive{} \configvalid}{The \gls{user} clicks on the button labeled "Start Recording". The user-interface-managing component opens an information dialog.}{A dialog is being displayed, advising the \gls{user} not to enter any personal or confidential business information.}
    \fspec{\Gls{user} controllability (stop)}{\Gls{user}}{\sessionactive}{The \gls{user} clicks on the button labeled "Stop recording". The recording-managing component will stop the current \gls{session}-recording.}{\sessioninactive{} The user-interface-managing component shows the save-dialog to the \gls{user}, providing the options to store or discard the recorded session. For UI details, see \hyperref[sec:uispec]{Save dialog}.} %%TODO: link UI
    ...
    \fspec{Quit while recording}{\Gls{user}}{\applaunched{} \sessionactive}{The \gls{user} chooses the "Quit" option while a \gls{session} is being recorded. The user-interface-managing-component opens the save-dialog.}{The save-dialog is shown to the \gls{user}. The application terminates immediately if the \gls{user} decided to discard the recording or terminates after the recording has been saved by the recording-managing component if the \gls{user} decided to save the recording.}
    \fspec{Save prompt}{\Gls{user}}{A \gls{session}-recording has been stopped and the save-dialog is displayed.}{The \gls{user} chooses whether to save or discard the recorded \gls{session}.}{If the \gls{user} chose to save the recording, the recording is stored by the recording-managing component at the location specified in the configuration file. If the \gls{user} chose the discard-option, the recording is not stored.}
    \fspec{\Gls{user} reviewability}{\Gls{user}}{\sessionsavail}{A \gls{user} clicks on the "Open recordings folder" button. The user-interface-managing component opens the recordings folder in the Windows File Explorer, displaying the files in which this \gls{user}'s past recordings have been stored.}{The folder containing the \gls{user}'s recordings is opened in the Windows File Explorer. The \gls{user} can view the videostream contained in these files by opening them with a compatible video-player, e.g. Windows Media Player.}
\end{itemize}
...
\begin{itemize}
    \fspec[200]{Videostream recording}{\Gls{user}}{\sessionactive}{While a \gls{session} is being recorded, the video-recording-managing component constantly captures the \gls{device}'s primary screen's content into a \gls{videostream} which the recording-managing component then serializes. The \gls{videostream} forms the basis for every recording.}{The \gls{session} recording contains a \gls{videostream} containing the \gls{device}'s primary screen's content.}
...
\begin{itemize}
    ...
    \fspec{Error dialog}{\Gls{user}}{\configinvalid}{The \gls{user} starts the application. The configuration-managing component will detect syntactic errors in the configuration file and the user-interface-managing component will display an error-message.}{An error-dialog is shown, informing that the current configuration is invalid. When the \gls{user} clicks the "OK" button shown in this dialog, the application will terminate.}
    \fspec{Save dialog}{\Gls{user}}{\sessionactive}{The \gls{user} stops the recording by pressing the "Stop recording" button or the "Quit" button. The user-interface-managing component opens a new dialog.}{A dialog window is shown to the \gls{user}, providing the options "Save recording" and "Discard recording".}
    ...
    \fspec{Tray-Icon}{\Gls{user}}{\configvalid}{The \gls{user} starts the application. The user-interface-managing component adds a new icon to the tray, indicating that the software has been launched successfully.}{An additional icon is shown in the system tray.}
    \fspec{Tray-Menu}{\Gls{user}}{\applaunched}{The \gls{user} clicks on the tray-icon. The user-interface-managing component shows a menu providing the following options:
    \begin{itemize}
