...
\begin{specifications}
    \spec{Global configuration\\The application core is responsible for parsing configuration files and providing the parsed configuration.}%%This overlaps with system module configuration, but it also includes for example video quality - any better way to phrase/organise it?
    ...
    \spec{Combinability of modules\\The application core needs to be able to function with arbitrary combinations of enabled modules. Performance impacts may occur if non-default modules are enabled.}%%Should the performance disclaimer be part of the functional specifications?
...
\begin{specifications}
    ...
    \begin{itemize}
        \item Collecting modules\\Modules which accept no input events and generate one or more output events
        \item Transforming modules\\Modules which accept one or more input events and generate one or more output events
        \item Discarding modules\\Modules which accept one or more input events and generate no output events
    \end{itemize}}
\end{specifications}
